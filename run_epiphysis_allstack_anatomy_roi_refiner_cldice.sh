...
REFINER_EXP="${REFINER_EXP:-allstack_anatomy_roi_refiner_sam_hqsam_cldice}"
REFINER_CHECKPOINT="${REFINER_CHECKPOINT:-${ERROR_ROOT}/best_allstack_anatomy_roi_sam_hqsam_cldice.pt}"
THRESHOLD_JSON="${THRESHOLD_JSON:-${ERROR_ROOT}/threshold_sweep_val_allstack_anatomy_roi_sam_hqsam_cldice.json}"
THRESHOLDS="${THRESHOLDS:-0.44,0.46,0.48,0.50,0.52,0.54}"
CLDICE_LOSS_WEIGHT="${CLDICE_LOSS_WEIGHT:-0.10}"
CLDICE_ITERS="${CLDICE_ITERS:-3}"
...
python scripts/train_error_refiner.py \
  ...
  --cldice-loss-weight "${CLDICE_LOSS_WEIGHT}" \
  --cldice-iters "${CLDICE_ITERS}" \
...
python scripts/summarize_ablation_metrics.py \
  ...
  --experiments \
    ...
    "${REFINED_EXP}" \
    "${REFINED_EXP_2}" \
    "allstack_anatomy_roi_refiner_sam_hqsam" \
    "${REFINER_EXP}" \
  --output "${PRED_ROOT}/${DATASET}_test_allstack_anatomy_roi_refiner_cldice_summary.md"
