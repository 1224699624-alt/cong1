...
def gate_positive_completion(source: torch.Tensor, refined: torch.Tensor,
                             seam: torch.Tensor, overlap: torch.Tensor) -> torch.Tensor:
    """Fixed GT-free inference diagnostic: do not refill confident gaps.

    This is an inference policy candidate, not a claim of retrained matching
    optimization. It is deliberately recorded separately from the pilot.
    """
    s = normalize_prior(seam)
    support = (1 - s).square() + s * overlap.clamp(0, 1)
    change = refined - source
    return source + torch.minimum(change, torch.zeros_like(change)) + change.clamp_min(0) * support
