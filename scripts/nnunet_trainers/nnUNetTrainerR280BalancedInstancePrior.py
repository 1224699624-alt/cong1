"""R280: R260-style instance prior with balanced seam suppression and bone-side support."""
...
def weighted_case_mean(values: torch.Tensor, weights: torch.Tensor) -> torch.Tensor:
    numerator = (values * weights).flatten(1).sum(1)
    denominator = weights.flatten(1).sum(1)
    valid = denominator > 1e-6
    return ((numerator[valid] / (denominator[valid] + 1e-6)).mean()
            if valid.any() else values.sum() * 0)


class nnUNetTrainerR280BalancedInstancePrior(nnUNetTrainer):
    """Suppress foreground on seam centers while protecting both labeled bone flanks."""
    ...
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict,
        ...
        self.num_epochs = 60
        self.seam_alpha = 0.05
        self.support_alpha = 0.05
        self.support_radius = 4
        ...
        torch.cuda.manual_seed_all(280)

    def initialize(self):
        if self.was_initialized:
            return
        if os.environ.get("nnUNet_compile", "").lower() != "false":
            raise RuntimeError("R280 requires nnUNet_compile=false for strict mature loading")
        super().initialize()
        path = Path(os.environ["R280_ADAPTED_CHECKPOINT"])
        ...
        manifest = {
            ...
            "seam_alpha": self.seam_alpha,
            "support_alpha": self.support_alpha,
            "support_radius_px": self.support_radius,
            "clean_test_used": False,
        }
        (Path(self.output_folder) / "r280_initialization.json").write_text(
            json.dumps(manifest, indent=2), encoding="utf-8")
        self.print_to_log_file("R280 mature initialization", json.dumps(manifest))
    ...
    def on_train_epoch_end(self, outputs: list[dict]):
        ...
        with (Path(self.output_folder) / "r280_loss_dynamics.jsonl").open("a", encoding="utf-8") as handle:
    ...
    def on_epoch_end(self):
        ...
        (Path(self.output_folder) / "r280_early_stop.json").write_text(
            json.dumps(state, indent=2), encoding="utf-8")
        self.print_to_log_file("R280 early stop", json.dumps(state))
