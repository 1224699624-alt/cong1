#!/usr/bin/env python3
"""R240 GT-free background-channel geometry editor.
...
R228 seam-probability map. R240 changes R239's selection policy: seam
probability is no longer a hard gate; local background-channel geometry is the
primary deployable filter and seam probability is only a ranking feature. GT is
used only after candidate generation for train/val evaluation/audit. Do not use
this on clean-test-v2 for tuning.
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run R240 GT-free bg-channel geometry editor on original val.")
    ...
    parser.add_argument("--output-exp", default="r240_bg_channel_geometry_editor")
    ...
    parser.add_argument("--selection-mode", choices=["r239-prob", "geometry"], default="geometry")
    parser.add_argument("--use-audit-safety-gate", action="store_true", help="Use GT-derived metric deltas as an additional development-only safety gate.")
    ...
    parser.add_argument("--max-cut-area-abs", type=int, default=24)
    parser.add_argument("--min-cut-mean-grad", type=float, default=0.0)
    ...
    parser.add_argument("--summary-json", type=Path, default=Path("outputs/analysis/r240_bg_channel_geometry_summary.json"))
    parser.add_argument("--per-image-csv", type=Path, default=Path("outputs/analysis/r240_bg_channel_geometry_per_image.csv"))
    parser.add_argument("--candidate-csv", type=Path, default=Path("outputs/analysis/r240_bg_channel_geometry_candidates.csv"))
    parser.add_argument("--seed", type=int, default=202607240)
...
def gate_candidate(row: dict[str, Any], args: argparse.Namespace) -> tuple[bool, str]:
    if args.selection_mode == "r239-prob":
    ...
    if args.selection_mode == "geometry":
        if as_float(row, "bg_channel_proxy") <= 0.5:
            return False, "no_bg_channel_proxy"
        if as_float(row, "ring_bg_frac") < args.min_ring_bg_frac:
            return False, "low_ring_bg_frac"
        if as_float(row, "ring_bg_frac") > args.max_ring_bg_frac:
            return False, "high_ring_bg_frac"
        if as_float(row, "cut_mean_dist_in") > args.max_cut_mean_dist_in:
            return False, "deep_mean_cut"
        if as_float(row, "cut_max_dist_in") > args.max_cut_max_dist_in:
            return False, "deep_max_cut"
        if as_float(row, "cut_area") > args.max_cut_area_abs:
            return False, "large_cut_area"
        if as_float(row, "cut_mean_grad") < args.min_cut_mean_grad:
            return False, "low_cut_grad"
        if as_float(row, "bridge_score_p90") > args.max_bridge_score_p90:
            return False, "high_bridge_score"
    if args.use_audit_safety_gate:
        ...
        for _score, pair_kind, info_a, info_b in build_pair_specs(args, anchor, component):
            ...
            for radius in parse_int_list(args.corridor_radii):
                ...
                for action_frac in parse_float_list(args.action_fracs):
                    ...
                    row: dict[str, Any] = {
                        "run_id": "R240-bg-channel-geometry-editor",
                        "split": args.split,
                        "image": name,
...
def run(args: argparse.Namespace) -> tuple[dict[str, Any], list[dict[str, Any]]]:
    ...
    for name in tqdm(app_names, desc=f"r240/apply/{args.split}"):
    ...
    report = {
        ...
        "candidate_generation": "GT-free bg components/seeds + seam probability; GT only for val metrics/audit",
        "selection_mode": args.selection_mode,
        "selection_note": "geometry mode uses deployable background-channel features as hard filters and seam probability only for ranking",
        "use_audit_safety_gate": bool(args.use_audit_safety_gate),
