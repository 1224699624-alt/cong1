#!/usr/bin/env python3
"""Prepare COCO data with one binary union epiphysis mask per image."""
...
def convert_split(raw_root: Path, dataset: str, split: str, count: int, max_size: int, out_root: Path) -> dict:
    ...
    return {
        ...
        "categories": [{"id": 1, "name": "epiphysis_union", "supercategory": "bone"}],
        "_summary": {"skipped_empty": skipped_empty},
    }
...
def main() -> None:
    ...
    for split, count in (("train", args.train_count), ("val", args.val_count)):
        ...
        summary[split] = {
            ...
            "skipped_empty": private["skipped_empty"],
            "json": str(ann_path),
            "image_root": str(out_root / split),
        }
    summary["constraints"] = {
        "source": f"{args.raw_root}/{args.dataset}",
        "label_transform": "all positive instance ids converted to one binary union mask per image",
        "clean_test_v2_used": False,
        "new_or_reannotated_test_used": False,
        "max_size": args.max_size,
    }
    summary_path = out_root.parent / "r196_union_coco_summary.json"
