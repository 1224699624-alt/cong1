#!/usr/bin/env python3
"""Build R211-F2 original-val hard-case visualization panels."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build R211-F2 val hard-case panels.")
    parser.add_argument("--manifest-csv", type=Path, default=Path("outputs/analysis/r211_f2_remote_fullval_visual_candidates.csv"))
    parser.add_argument("--metrics-csv", type=Path, default=Path("outputs/analysis/r211_f2_remote_fullval_val_per_image.csv"))
    ...
    parser.add_argument("--r211-dir", type=Path, default=Path("outputs/ablations_variants/r211_f2_component_guard_remote_fullval/TSRS_RSNA-Epiphysis/val/masks"))
    parser.add_argument("--output-dir", type=Path, default=Path("outputs/analysis/r211_f2_val_hardcase_visualizations"))
...
def edit_overlay(gray: np.ndarray, r110: np.ndarray, r211: np.ndarray, gt: np.ndarray) -> Image.Image:
    base = np.stack([gray, gray, gray], axis=-1).astype(np.float32)
    removed = r110 & ~r211
    added = r211 & ~r110
    gt_b = boundary(gt)
    r211_b = boundary(r211)
...
def row_reason(row: dict[str, str]) -> str:
    return (
        f"{row.get('group', '')} rank {row.get('rank', '')}; "
        f"gap {float(row.get('delta_gap_region_fp_rate', 0.0)):+.4f}, "
        f"BIoU {float(row.get('delta_boundary_iou', 0.0)):+.4f}, "
        f"recall {float(row.get('delta_recall', 0.0)):+.4f}, "
        f"component MAE {float(row.get('delta_component_count_mae', 0.0)):+.4f}"
    )
...
def make_panel(row: dict[str, str], metrics: dict[str, str], args: argparse.Namespace) -> Image.Image:
    ...
    r211 = read_mask(args.r211_dir / name, gray.shape)
    metric_sub = (
    ...
    panels = [
        ...
        label_panel(resize_panel(tint_overlay(gray, r211, gt), args.panel_width), "R211-F2", metric_sub),
        label_panel(resize_panel(edit_overlay(gray, r110, r211, gt), args.panel_width), "Accepted edit", "removed orange, added cyan"),
    ]
...
def write_html(index_rows: list[dict[str, Any]], output_dir: Path) -> None:
    parts = [
        "<!doctype html><html><head><meta charset='utf-8'><title>R211-F2 val hard cases</title>",
        ...
        "</head><body><h1>R211-F2 Original-Val Hard Cases</h1>",
        "<p>Train/val-only audit. Legend: GT boundary green, prediction boundary magenta, TP blue tint, FP red, FN yellow. Accepted deletions are orange.</p>",
    ]
...
def main() -> None:
    ...
    (args.output_dir / "r211_f2_val_hardcase_visualization_index.json").write_text(json.dumps(index_rows, indent=2), encoding="utf-8")
    with (args.output_dir / "r211_f2_val_hardcase_visualization_index.csv").open("w", newline="", encoding="utf-8") as handle:
        writer = csv.DictWriter(handle, fieldnames=["rank", "image", "group", "reason", "panel"])
        writer.writeheader()
        for row in index_rows:
