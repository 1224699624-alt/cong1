...
REFINER_EXP="${REFINER_EXP:-single_candidate_sam_roi_prior_refiner}"
REFINER_CHECKPOINT="${REFINER_CHECKPOINT:-${ERROR_ROOT}/best_single_candidate_sam_roi_prior_refiner.pt}"
THRESHOLD_JSON="${THRESHOLD_JSON:-${ERROR_ROOT}/threshold_sweep_val_single_candidate_sam_roi_prior_refiner.json}"
...
python scripts/train_error_refiner.py \
  ...
  --target-mode allstack_anatomy_roi_prior_only \
  --img-size 512 \
  --epochs 80 \
  --min-epochs 20 \
  --patience 10 \
...
python scripts/summarize_ablation_metrics.py \
  ...
  --experiments \
    ...
    "${REFINED_EXP}" \
    "${REFINER_EXP}" \
  --output "${PRED_ROOT}/${DATASET}_test_single_candidate_sam_roi_prior_summary.md"
