...
REFINER_EXP="${REFINER_EXP:-single_candidate_sam_roi_only_refiner}"
REFINER_CHECKPOINT="${REFINER_CHECKPOINT:-${ERROR_ROOT}/best_single_candidate_sam_roi_only_refiner.pt}"
THRESHOLD_JSON="${THRESHOLD_JSON:-${ERROR_ROOT}/threshold_sweep_val_single_candidate_sam_roi_only_refiner.json}"
...
python scripts/summarize_ablation_metrics.py \
  ...
  --experiments \
    ...
    "${REFINED_EXP}" \
    "${REFINER_EXP}" \
  --output "${PRED_ROOT}/${DATASET}_test_single_candidate_sam_roi_only_summary.md"
