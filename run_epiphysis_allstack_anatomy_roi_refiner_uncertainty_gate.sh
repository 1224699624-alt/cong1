...
REFINER_EXP="${REFINER_EXP:-allstack_anatomy_roi_refiner_uncertainty_gate_sam_hqsam}"
REFINER_CHECKPOINT="${REFINER_CHECKPOINT:-${ERROR_ROOT}/best_allstack_anatomy_roi_refiner_uncertainty_gate_sam_hqsam.pt}"
THRESHOLD_JSON="${THRESHOLD_JSON:-${ERROR_ROOT}/threshold_sweep_val_allstack_anatomy_roi_refiner_uncertainty_gate_sam_hqsam.json}"
...
python scripts/train_error_refiner.py \
  ...
  --target-mode allstack_anatomy_roi_refiner_uncertainty_gate \
  --img-size 512 \
  --epochs 80 \
  --min-epochs 20 \
  --patience 10 \
...
python scripts/summarize_ablation_metrics.py \
  ...
  --experiments \
    ...
    "${REFINED_EXP_2}" \
    "${REFINER_EXP}" \
  --output "${PRED_ROOT}/${DATASET}_test_allstack_anatomy_roi_refiner_uncertainty_gate_summary.md"
