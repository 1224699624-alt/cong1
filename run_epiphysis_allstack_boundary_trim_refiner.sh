...
REFINER_EXP="${REFINER_EXP:-allstack_boundary_trim_refiner_sam_hqsam}"
REFINER_CHECKPOINT="${REFINER_CHECKPOINT:-${ERROR_ROOT}/best_allstack_boundary_trim_sam_hqsam.pt}"
THRESHOLD_JSON="${THRESHOLD_JSON:-${ERROR_ROOT}/threshold_sweep_val_allstack_boundary_trim_sam_hqsam.json}"
THRESHOLDS="${THRESHOLDS:-0.44,0.46,0.48,0.50,0.52,0.54,0.56}"
...
python scripts/train_error_refiner.py \
  ...
  --target-mode allstack_boundary_trim \
  --img-size 512 \
  --epochs 80 \
  --min-epochs 20 \
  --patience 10 \
...
python scripts/summarize_ablation_metrics.py \
  ...
  --experiments \
    ...
    "${REFINED_EXP_2}" \
    "${REFINER_EXP}" \
  --output "${PRED_ROOT}/${DATASET}_test_allstack_boundary_trim_refiner_summary.md"
