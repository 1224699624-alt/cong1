...
run_error_refiner_variant() {
  ...
  local threshold_json_name="$3"
  local lr="$4"
  local base_channels="$5"
  local aux_loss_weight="$6"
...
run_error_refiner_variant \
  "hq_sam_error_refiner_iter2_pad003_neg2_k9" \
  "best_hq_sam_iter2_pad003_neg2_k9.pt" \
  "threshold_sweep_val_hq_sam_iter2_pad003_neg2_k9.json" \
  "5e-4" \
  "32" \
  "0.35"

run_error_refiner_variant \
  "hq_sam_error_refiner_iter2_aux020" \
  "best_hq_sam_iter2_aux020.pt" \
  "threshold_sweep_val_hq_sam_iter2_aux020.json" \
  "5e-4" \
  "32" \
  "0.20"

run_error_refiner_variant \
  "hq_sam_error_refiner_iter2_bc24_lr3e4" \
  "best_hq_sam_iter2_bc24_lr3e4.pt" \
  "threshold_sweep_val_hq_sam_iter2_bc24_lr3e4.json" \
  "3e-4" \
  "24" \
  "0.35"
...
python scripts/summarize_ablation_metrics.py \
  ...
  --experiments \
    ...
    "hq_sam_error_refiner_iter2_aux020" \
    "hq_sam_error_refiner_iter2_bc24_lr3e4" \
  --output "${PRED_ROOT}/${DATASET}_test_hq_sam_error_refiner_tuning_summary.md"
