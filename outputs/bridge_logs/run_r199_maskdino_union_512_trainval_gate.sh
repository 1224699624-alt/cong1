...
OUT_DIR="outputs/analysis/r199_maskdino_union_512_trainval_gate"
...
{
  echo "[R199] start $(date -Is)"
  echo "[R199] CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES}"
  nvidia-smi --query-gpu=index,name,memory.used,memory.total,utilization.gpu --format=csv,noheader || true

  if [[ ! -s "${WEIGHT_PATH}" ]]; then
    echo "[R199] missing pretrained weight: ${WEIGHT_PATH}" >&2
    exit 2
  fi
  ...
  cat > "${OUT_DIR}/r199_register_and_train.py" <<'PY'
import json
...
OUT = ROOT / "outputs/analysis/r199_maskdino_union_512_trainval_gate"
COCO = OUT / "coco"
register_coco_instances("epiphysis_r199_union_train", {}, str(COCO / "train.json"), str(COCO / "train"))
register_coco_instances("epiphysis_r199_union_val", {}, str(COCO / "val.json"), str(COCO / "val"))
...
args = default_argument_parser().parse_args([
    ...
    "DATASETS.TRAIN", "('epiphysis_r199_union_train',)",
    "DATASETS.TEST", "('epiphysis_r199_union_val',)",
...
summary = {
    ...
    "constraints": {
        ...
        "purpose": "server-only higher-resolution MaskDINO union train/val gate"
    }
}
(OUT / "r199_summary.json").write_text(json.dumps(summary, ensure_ascii=False, indent=2), encoding="utf-8")
...
PY
  ...
  cd "${MASKDINO_DIR}"
  "${PY}" "/home/shenzeyu/workspace/YOLO_SAM_generic_src/${OUT_DIR}/r199_register_and_train.py"
  cd /home/shenzeyu/workspace/YOLO_SAM_generic_src
  ...
  if [[ -f "${OUT_DIR}/maskdino_output/inference/coco_instances_results.json" ]]; then
    "${PY}" scripts/diagnose_r192_maskdino_predictions.py \
      ...
      --out-json "${OUT_DIR}/r199_val_prediction_diagnostic.json" \
      --top-k "1,2,3,5,10,20"

    "${PY}" scripts/evaluate_maskdino_union_constraints.py \
      --coco-json "${OUT_DIR}/coco/val.json" \
      --pred-json "${OUT_DIR}/maskdino_output/inference/coco_instances_results.json" \
      --out-json "${OUT_DIR}/r199_val_constraint_metrics.json" \
      --export-dir "${OUT_DIR}/val_selected_masks" \
      --top-k "1,2,3,5,10,20"
  fi

  echo "[R199] done $(date -Is)"
} 2>&1 | tee "${LOG_DIR}/r199_maskdino_union_512_trainval_gate.log"
