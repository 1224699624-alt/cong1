...
OUT_DIR="outputs/analysis/r195_maskdino_one_image_overfit_rescue"
...
{
  echo "[R195] start $(date -Is)"
  echo "[R195] CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES}"
  nvidia-smi --query-gpu=index,name,memory.used,memory.total,utilization.gpu --format=csv,noheader || true

  if [[ ! -s "${WEIGHT_PATH}" ]]; then
    echo "[R195] missing pretrained weight: ${WEIGHT_PATH}" >&2
    exit 2
  fi
  ...
  cat > "${OUT_DIR}/r195_register_and_train.py" <<'PY'
import json
...
OUT = ROOT / "outputs/analysis/r195_maskdino_one_image_overfit_rescue"
COCO = OUT / "coco"
register_coco_instances("epiphysis_r195_train", {}, str(COCO / "train.json"), str(COCO / "train"))
...
args = default_argument_parser().parse_args([
    ...
    "MODEL.MaskDINO.DN_NUM", "50",
    "MODEL.MaskDINO.TEST.OBJECT_MASK_THRESHOLD", "0.0001",
    "DATASETS.TRAIN", "('epiphysis_r195_train',)",
    "DATASETS.TEST", "('epiphysis_r195_train',)",
...
summary = {
    ...
    "constraints": {
        ...
        "purpose": "server-only one-image overfit target/config rescue"
    }
}
(OUT / "r195_summary.json").write_text(json.dumps(summary, ensure_ascii=False, indent=2), encoding="utf-8")
...
PY
  ...
  cd "${MASKDINO_DIR}"
  "${PY}" "/home/shenzeyu/workspace/YOLO_SAM_generic_src/${OUT_DIR}/r195_register_and_train.py"
  cd /home/shenzeyu/workspace/YOLO_SAM_generic_src
  ...
  if [[ -f "${OUT_DIR}/maskdino_output/inference/coco_instances_results.json" ]]; then
    "${PY}" scripts/diagnose_r192_maskdino_predictions.py \
      ...
      --out-json "${OUT_DIR}/r195_train_prediction_diagnostic.json"
  fi

  echo "[R195] done $(date -Is)"
} 2>&1 | tee "${LOG_DIR}/r195_maskdino_one_image_overfit_rescue.log"
