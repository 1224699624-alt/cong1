...
OUT_DIR="outputs/analysis/r198_maskdino_union_full_trainval_gate"
...
{
  echo "[R198] start $(date -Is)"
  echo "[R198] CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES}"
  nvidia-smi --query-gpu=index,name,memory.used,memory.total,utilization.gpu --format=csv,noheader || true

  if [[ ! -s "${WEIGHT_PATH}" ]]; then
    echo "[R198] missing pretrained weight: ${WEIGHT_PATH}" >&2
    exit 2
  fi
  ...
  cat > "${OUT_DIR}/r198_register_and_train.py" <<'PY'
import json
...
OUT = ROOT / "outputs/analysis/r198_maskdino_union_full_trainval_gate"
COCO = OUT / "coco"
register_coco_instances("epiphysis_r198_union_train", {}, str(COCO / "train.json"), str(COCO / "train"))
register_coco_instances("epiphysis_r198_union_val", {}, str(COCO / "val.json"), str(COCO / "val"))
...
args = default_argument_parser().parse_args([
    ...
    "DATASETS.TRAIN", "('epiphysis_r198_union_train',)",
    "DATASETS.TEST", "('epiphysis_r198_union_val',)",
    ...
    "SOLVER.STEPS", "()",
    "SOLVER.CHECKPOINT_PERIOD", "12000",
    "TEST.EVAL_PERIOD", "12000",
...
summary = {
    ...
    "constraints": {
        "train_source": "data/raw/TSRS_RSNA-Epiphysis/train full original",
        "val_source": "data/raw/TSRS_RSNA-Epiphysis/val full original",
        ...
        "purpose": "server-only full train/val binary union gate"
    }
}
(OUT / "r198_summary.json").write_text(json.dumps(summary, ensure_ascii=False, indent=2), encoding="utf-8")
...
PY
  ...
  cd "${MASKDINO_DIR}"
  "${PY}" "/home/shenzeyu/workspace/YOLO_SAM_generic_src/${OUT_DIR}/r198_register_and_train.py"
  cd /home/shenzeyu/workspace/YOLO_SAM_generic_src
  ...
  if [[ -f "${OUT_DIR}/maskdino_output/inference/coco_instances_results.json" ]]; then
    "${PY}" scripts/diagnose_r192_maskdino_predictions.py \
      ...
      --out-json "${OUT_DIR}/r198_val_prediction_diagnostic.json" \
      --top-k "1,2,3,5,10,20"

    "${PY}" scripts/evaluate_maskdino_union_constraints.py \
      --coco-json "${OUT_DIR}/coco/val.json" \
      --pred-json "${OUT_DIR}/maskdino_output/inference/coco_instances_results.json" \
      --out-json "${OUT_DIR}/r198_val_constraint_metrics.json" \
      --export-dir "${OUT_DIR}/val_selected_masks" \
      --top-k "1,2,3,5,10,20"
  fi

  echo "[R198] done $(date -Is)"
} 2>&1 | tee "${LOG_DIR}/r198_maskdino_union_full_trainval_gate.log"
