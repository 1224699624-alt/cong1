...
REFINER_EXP="${REFINER_EXP:-boundary_band_error_refiner_sam_hqsam}"
REFINER_CHECKPOINT="${REFINER_CHECKPOINT:-${ERROR_ROOT}/best_boundary_band_sam_hqsam.pt}"
THRESHOLD_JSON="${THRESHOLD_JSON:-${ERROR_ROOT}/threshold_sweep_val_boundary_band_sam_hqsam.json}"
THRESHOLDS="${THRESHOLDS:-0.38,0.40,0.42,0.45,0.48,0.50}"
...
python scripts/train_error_refiner.py \
  ...
  --target-mode boundary_band_fgbg \
  --img-size 512 \
  --epochs 80 \
  --min-epochs 20 \
  --patience 10 \
  ...
  --boundary-band-kernel 9 \
  --boundary-band-weight 1.0 \
...
python scripts/summarize_ablation_metrics.py \
  ...
  --experiments \
    ...
    "${REFINED_EXP_2}" \
    "${REFINER_EXP}" \
  --output "${PRED_ROOT}/${DATASET}_test_boundary_band_error_refiner_summary.md"
