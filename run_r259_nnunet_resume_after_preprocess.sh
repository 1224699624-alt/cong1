...
LOG="outputs/bridge_logs/r259_nnunet_resume_after_preprocess.log"; exec > >(tee -a "$LOG") 2>&1; echo "[R259-RESUME] start $(date -Is)"
test -s "$nnUNet_preprocessed/Dataset203_TSRS_RSNAEpiphysisPrior2D/nnUNetPlans.json"; test -s outputs/priors/r259_frozen_relation/manifest.json
TRAINER_DST="$($PY - <<'PY'
from pathlib import Path
import nnunetv2
print(Path(nnunetv2.__file__).parent/'training/nnUNetTrainer/variants/loss/nnUNetTrainerR259FrozenPrior.py')
PY
)"; cp scripts/nnunet_trainers/nnUNetTrainerR259FrozenPrior.py "$TRAINER_DST"; $PY -m py_compile "$TRAINER_DST"
$PY scripts/prepare_r259_val_inputs.py --native-dir "$RUN/imagesValNative" --active-dir "$RUN/imagesValPrior"
DS="Dataset203_TSRS_RSNAEpiphysisPrior2D"; W="$nnUNet_results/$DS/nnUNetTrainerR259Warmup__nnUNetPlans__2d/fold_0"; N="$nnUNet_results/$DS/nnUNetTrainerR259Native__nnUNetPlans__2d/fold_0"; A="$nnUNet_results/$DS/nnUNetTrainerR259FrozenPrior__nnUNetPlans__2d/fold_0"; rm -rf "${W%/fold_0}" "${N%/fold_0}" "${A%/fold_0}"
nnUNetv2_train 203 2d 0 -tr nnUNetTrainerR259Warmup
test -s "$W/checkpoint_final.pth"; mkdir -p "$N" "$A"; cp "$W/checkpoint_final.pth" "$N/checkpoint_latest.pth"; cp "$W/checkpoint_final.pth" "$A/checkpoint_latest.pth"; SHA="$(sha256sum "$W/checkpoint_final.pth"|cut -d' ' -f1)"; test "$SHA" = "$(sha256sum "$N/checkpoint_latest.pth"|cut -d' ' -f1)"; test "$SHA" = "$(sha256sum "$A/checkpoint_latest.pth"|cut -d' ' -f1)"
...
test -s "$A/r259_prior_dynamics.jsonl"; rm -rf "$RUN/predictions_native" "$RUN/predictions_prior"
...
$PY scripts/convert_nnunet_predictions.py --pred-dir "$RUN/predictions_native" --mask-dir "$NATIVE" --expected-count 96 --strip-prefix val_; $PY scripts/convert_nnunet_predictions.py --pred-dir "$RUN/predictions_prior" --mask-dir "$ACTIVE" --expected-count 96 --strip-prefix val_
$PY scripts/evaluate_r201_single_split.py --gt-dir data/raw_variants/TSRS_RSNA-Epiphysis_contrast_v1/val_labels --pred-dir "$NATIVE" --output outputs/analysis/r259_native_original_val_r201.json --expected-count 96; $PY scripts/evaluate_r201_single_split.py --gt-dir data/raw_variants/TSRS_RSNA-Epiphysis_contrast_v1/val_labels --pred-dir "$ACTIVE" --output outputs/analysis/r259_frozen_prior_original_val_r201.json --expected-count 96
...
import json
from pathlib import Path
p=Path('outputs/visualizations/r259_nnunet_frozen_prior_original_val'); assert len(json.loads((p/'manifest.json').read_text()))==12 and len(list(p.glob('*_comparison.png')))==12
PY
echo "[R259-RESUME] done $(date -Is)"
