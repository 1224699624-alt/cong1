...
REFINER_EXP="${REFINER_EXP:-allstack_anatomy_roi_refiner_sam_hqsam}"
REFINER_CHECKPOINT="${REFINER_CHECKPOINT:-${ERROR_ROOT}/best_allstack_anatomy_roi_sam_hqsam.pt}"
THRESHOLD_JSON="${THRESHOLD_JSON:-${ERROR_ROOT}/threshold_sweep_val_allstack_anatomy_roi_sam_hqsam.json}"
THRESHOLDS="${THRESHOLDS:-0.44,0.46,0.48,0.50,0.52,0.54}"
POST_MEDIAN_KSIZE="${POST_MEDIAN_KSIZE:-3}"
POST_OPEN_KERNEL="${POST_OPEN_KERNEL:-3}"
POST_MIN_COMPONENT="${POST_MIN_COMPONENT:-48}"
POST_MIN_HOLE="${POST_MIN_HOLE:-24}"
...
python scripts/train_error_refiner.py \
  ...
  --target-mode allstack_anatomy_roi_refiner \
  --img-size 512 \
  --epochs 80 \
  --min-epochs 20 \
  --patience 10 \
...
python scripts/select_error_refiner_threshold.py \
  ...
  --selection-precision-weight 0.25 \
  --selection-boundary-weight 0.20 \
  --post-median-ksize "${POST_MEDIAN_KSIZE}" \
  --post-open-kernel "${POST_OPEN_KERNEL}" \
  --post-min-component "${POST_MIN_COMPONENT}" \
  --post-min-hole "${POST_MIN_HOLE}" \
...
python scripts/summarize_ablation_metrics.py \
  ...
  --experiments \
    ...
    "${REFINED_EXP_2}" \
    "${REFINER_EXP}" \
  --output "${PRED_ROOT}/${DATASET}_test_allstack_anatomy_roi_refiner_summary.md"
