...
ITER_REFINED_EXP="zero_shot_mask_to_prompt_refine_iter3_pad003_neg2_k5"
ERROR_REFINER_EXP="error_refiner_iter3_unet512_b32"
ERROR_REFINER_CKPT="outputs/error_refiner/${DATASET}/best_iter3.pt"
...
python scripts/select_error_refiner_threshold.py \
  ...
  --baseline-exp "${BASELINE_EXP}" \
  --refined-exp "${ITER_REFINED_EXP}" \
  --checkpoint "${ERROR_REFINER_CKPT}" \
  --thresholds "0.40,0.45,0.50,0.55,0.60"

SELECTED_THRESHOLD="$(python - <<'PY'
import json
from pathlib import Path
path = Path("outputs/error_refiner/TSRS_RSNA-Epiphysis/threshold_sweep_val.json")
...
python scripts/infer_error_refiner.py \
  ...
  --refined-exp "${ITER_REFINED_EXP}" \
  --checkpoint "${ERROR_REFINER_CKPT}" \
  --output-name "${ERROR_REFINER_EXP}" \
  --threshold "${SELECTED_THRESHOLD}"

python scripts/evaluate_masks.py \
  --dataset "${DATASET}" \
  --split test \
  --pred-dir "${PRED_ROOT}/${ERROR_REFINER_EXP}/${DATASET}/test/masks"
...
python scripts/summarize_ablation_metrics.py \
  ...
  --experiments \
    ...
    "${ITER_REFINED_EXP}" \
    "${ERROR_REFINER_EXP}" \
  --output "${PRED_ROOT}/${DATASET}_test_refiner_path_summary.md"
