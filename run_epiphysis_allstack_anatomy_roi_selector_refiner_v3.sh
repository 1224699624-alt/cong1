...
REFINER_EXP="${REFINER_EXP:-allstack_anatomy_roi_selector_refiner_v3_sam_hqsam}"
REFINER_CHECKPOINT="${REFINER_CHECKPOINT:-${ERROR_ROOT}/best_allstack_anatomy_roi_selector_refiner_v3_sam_hqsam.pt}"
THRESHOLD_JSON="${THRESHOLD_JSON:-${ERROR_ROOT}/threshold_sweep_val_allstack_anatomy_roi_selector_refiner_v3_sam_hqsam.json}"
...
python scripts/train_error_refiner.py \
  ...
  --target-mode allstack_anatomy_roi_selector_refiner_v3 \
  --img-size 512 \
  --epochs 60 \
  --min-epochs 20 \
  --patience 10 \
  ...
  --selector-loss-weight 0.20 \
  --gate-disagreement-threshold 0.10 \
  --gate-boundary-weight 0.75 \
  --gate-uncertainty-weight 0.35 \
  --gate-smooth-kernel 5 \
  --boundary-fg-weight 0.60 \
  --boundary-bg-weight 0.50 \
  --selection-dice-weight 0.28 \
  --selection-iou-weight 0.22 \
  --selection-precision-weight 0.22 \
...
echo "Selected threshold: ${SELECTED_THRESHOLD}"
...
python scripts/summarize_ablation_metrics.py \
  ...
  --experiments \
    ...
    "${REFINED_EXP_2}" \
    "${REFINER_EXP}" \
  --output "${PRED_ROOT}/${DATASET}_test_allstack_anatomy_roi_selector_refiner_v3_summary.md"
