...
RESULT="outputs/analysis/r256_scale_invariant_pair_prior_sanity_${STAMP}.json"
"$PYTHON" scripts/train_r256_scale_invariant_pair_prior.py \
  ...
  --output-dir outputs/pair_prior/r256_scale_invariant_pair_prior_sanity \
  --result-json "$RESULT" \
  --limit-train 32 --limit-val 16 --epochs 1 --train-seeds 256 \
  --device cuda
cp "$RESULT" outputs/analysis/r256_scale_invariant_pair_prior_sanity.json
