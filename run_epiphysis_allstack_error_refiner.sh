...
REFINER_EXP="${REFINER_EXP:-allstack_error_refiner_sam_hqsam_gated_mec}"
REFINER_CHECKPOINT="${REFINER_CHECKPOINT:-${ERROR_ROOT}/best_allstack_sam_hqsam_gated_mec.pt}"
THRESHOLD_JSON="${THRESHOLD_JSON:-${ERROR_ROOT}/threshold_sweep_val_allstack_sam_hqsam_gated_mec.json}"
...
python scripts/train_error_refiner.py \
  ...
  --target-mode residual_mec_gated \
  --img-size 512 \
  --epochs 80 \
  --min-epochs 20 \
  --patience 10 \
...
python scripts/summarize_ablation_metrics.py \
  ...
  --experiments \
    ...
    "${REFINED_EXP_2}" \
    "${REFINER_EXP}" \
  --output "${PRED_ROOT}/${DATASET}_test_allstack_error_refiner_summary.md"
