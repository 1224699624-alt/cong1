...
RUN="$ROOT/outputs/nnunet/r317_image_centers_only"
PRIOR_MODELS="$ROOT/outputs/pair_prior/r317_image_centers_only"
PRIORS="$ROOT/outputs/priors/r317_image_centers_only"
R260_MASKS="$ROOT/outputs/ablations/r260_mature_prior/TSRS_RSNA-Epiphysis/val/masks"
REUSE_LOG="$ROOT/outputs/bridge_logs/r317_continuous_maturity.log"
LOG="$ROOT/outputs/bridge_logs/r317_image_centers_only.log"
CURRENT_LOG="$ROOT/outputs/bridge_logs/r317_image_centers_only.log"

cd "$ROOT"
mkdir -p outputs/bridge_logs outputs/analysis "$PRIOR_MODELS"
exec > >(tee -a "$LOG") 2>&1
...
$PY -m py_compile scripts/train_r258b_prediction_relation_prior.py scripts/build_r259_frozen_prior_maps.py

echo "[R317-IC] train/reuse image+centers prior only $(date -Is)"
rm -rf "$PRIORS" "$RUN"
$PY scripts/train_r258b_prediction_relation_prior.py \
  --r316c --r317 --image-centers-only \
  --reuse-completed-log "$REUSE_LOG" --reuse-completed-log "$CURRENT_LOG" \
  --crop-size 256 --global-size 128 \
  ...
  --result-json outputs/analysis/r317_image_centers_only_prior_convergence.json \
  --pair-manifest-csv outputs/analysis/r317_image_centers_only_pairs.csv

echo "[R317-IC] build frozen image+centers prior maps $(date -Is)"
$PY scripts/build_r259_frozen_prior_maps.py \
  ...
  --r258b-result outputs/analysis/r317_image_centers_only_prior_convergence.json \
...
echo "[R317-IC] prepare isolated nnU-Net data $(date -Is)"
...
echo "[R317-IC] train alpha=0.035 with periodic checkpoints $(date -Is)"
...
evaluate_checkpoint() {
  ...
  echo "[R317-IC] evaluate checkpoint=$label $(date -Is)"
  rm -rf "$pred" "$masks"
  ...
  $PY scripts/evaluate_r201_single_split.py \
    ...
    --output "outputs/analysis/r317_image_centers_only_${label}_original_val_r201.json" --expected-count 96
  rm -rf "$pred"
}
...
echo "[R317-IC] R201 hard-gate checkpoint selection $(date -Is)"
$PY scripts/summarize_r317_checkpoint_selection.py \
  --metrics-glob 'outputs/analysis/r317_image_centers_only_*_original_val_r201.json' \
  --trainer-dir "$FOLD" --output outputs/analysis/r317_image_centers_only_checkpoint_selection.json
SELECTED_LABEL="$($PY -c "import json; print(json.load(open('outputs/analysis/r317_image_centers_only_checkpoint_selection.json'))['selected']['label'])")"
SELECTED_CKPT="$($PY -c "import json; print(json.load(open('outputs/analysis/r317_image_centers_only_checkpoint_selection.json'))['selected']['checkpoint'])")"
...
$PY scripts/render_r259_nnunet_prior_comparison.py \
  ...
  --output-dir outputs/visualizations/r317_image_centers_only_vs_r260 \
  --num-cases 12 --baseline-label R260 --improved-label "R317 image+centers selected"
