...
REFINER_CHECKPOINT="${REFINER_CHECKPOINT:-${ERROR_ROOT}/best_allstack_anatomy_roi_keepbone_cutgap_refiner_v2_recall_rescue_sam_hqsam.pt}"
THRESHOLD_JSON="${THRESHOLD_JSON:-${ERROR_ROOT}/threshold_sweep_val_allstack_anatomy_roi_keepbone_cutgap_refiner_v2_recall_rescue_sam_hqsam.json}"
...
python scripts/train_error_refiner.py \
  ...
  --target-mode allstack_anatomy_roi_keepbone_cutgap_refiner_v2_recall_rescue \
  --img-size 512 \
  --epochs 80 \
  --min-epochs 20 \
  --patience 10 \
  ...
  --structure-keep-loss-weight 1.18 \
  ...
  --selection-dice-weight 0.34 \
  --selection-iou-weight 0.25 \
  --selection-precision-weight 0.18 \
  --selection-boundary-weight 0.23 \
...
python scripts/summarize_ablation_metrics.py \
  ...
  --experiments \
    ...
    "${REFINED_EXP_2}" \
    "${REFINER_EXP}" \
  --output "${PRED_ROOT}/${DATASET}_test_${REFINER_EXP}_summary.md"
