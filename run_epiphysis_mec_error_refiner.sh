...
BASELINE_EXP="${BASELINE_EXP:-hq_sam_box_only_conf020_pad005}"
REFINED_EXP="${REFINED_EXP:-hq_sam_refine_iter2_boxpointmask_pad003_neg2_k9}"
REFINED_EXP_2="${REFINED_EXP_2:-zero_shot_mask_to_prompt_refine_pad003_neg2_k5}"

REFINER_EXP="${REFINER_EXP:-mec_error_refiner_sam_hqsam}"
REFINER_CHECKPOINT="${REFINER_CHECKPOINT:-${ERROR_ROOT}/best_mec_error_refiner_sam_hqsam.pt}"
THRESHOLD_JSON="${THRESHOLD_JSON:-${ERROR_ROOT}/threshold_sweep_val_mec_error_refiner_sam_hqsam.json}"
THRESHOLDS="${THRESHOLDS:-0.35,0.40,0.45,0.50,0.55,0.60,0.65}"
...
python scripts/summarize_ablation_metrics.py \
  ...
  --experiments \
    ...
    "${REFINED_EXP_2}" \
    "${REFINER_EXP}" \
  --output "${PRED_ROOT}/${DATASET}_test_mec_error_refiner_summary.md"
