...
REFINER_EXP="${REFINER_EXP:-allstack_anatomy_roi_keepbone_cutgap_refiner_v3_sam_hqsam}"
REFINER_CHECKPOINT="${REFINER_CHECKPOINT:-${ERROR_ROOT}/best_allstack_anatomy_roi_keepbone_cutgap_refiner_v3_sam_hqsam.pt}"
THRESHOLD_JSON="${THRESHOLD_JSON:-${ERROR_ROOT}/threshold_sweep_val_allstack_anatomy_roi_keepbone_cutgap_refiner_v3_sam_hqsam.json}"
THRESHOLDS="${THRESHOLDS:-0.48,0.50,0.52,0.54,0.56,0.58}"
...
python scripts/train_error_refiner.py \
  ...
  --target-mode allstack_anatomy_roi_keepbone_cutgap_refiner_v3 \
  --img-size 512 \
  --epochs 80 \
  --min-epochs 20 \
  --patience 10 \
